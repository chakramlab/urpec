function [fieldsFileName,T] = urpec_v6( config )
% V6 (2026-08-30): TONE-AVERAGE SEMANTICS. Identical to urpec_v5
% except that each band tone entry of layerTargets is a promise
% about that tone AVERAGE absorbed dose: an outer loop measures the
% averages after each gradient pass and moves the internal per-pixel
% ask by the miss (warm restart). See the v6 block in the gradient
% solver. New config: outerIters, toneAvgTol, innerIters2.
...
if config.gradSolver
    if isempty(config.layerTargets)
        ...
        %########## v6: tone-average outer loop ##########
        %shapeW carries the INTERNAL asks; shape keeps the promise (and
        %is what exports/targetMap record). Tone identity is frozen
        %from the original shape, so an ask driven to 0 does not
        %reclassify pixels as outside.
        shapeW=shape;
        uWant=unique(gather(shape(isBand)));
        uWant=uWant(:)';
        uAsk=uWant;
        nT=numel(uWant);
        toneMask=cell(1,nT);
        ...
        for it=1:nIt
            ...
            vB=gather(sum(max(0,shapeW(isBand)-a(isBand)).^2,'all'));
        ...
        for tg=1:nT
            ...
            if nu<=0
                ...
                if uAsk(tg)<=0
                    pinned(tg)=true;
                    warning('urpec:toneFloor',['v6: tone target %.3f '...
                        'is below the spillover floor (avg %.3f at '...
                        'ask 0). Dose cannot fix this; change '...
                        'geometry or stack.'],uWant(tg),avgT);
                end
            end
            uAsk(tg)=nu;
